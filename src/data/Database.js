...
async function getGradeData(grade) {
  ...
  const result = await getCacheObject(grade)
    .then((result) => {
  ...
  await db
    .collection(grade)
    .get() // Pulling the grade data and storing it to the above list
    .then((snapshot) => {
...
async function getLessonsData(grade, chpt, languageCode) {
  ...
  const result = await getCacheObject(`${grade}-${chpt}-${languageCode}`)
    .then((result) => {
      console.log("Result: ", result);
      return result;
    })
  ...
  let i = 1;
  while (i < 50) {
    //safeguarding against infinite loops. 50 is an arbitrary number/cap
    try {
      ...
      if (!lessonSnapshot.exists) {
        break;
      } // Break the loop if the document doesn't exist

      let lessonObject = {
        //initializing lessonObject
        backgroundColor: "",
        navigation: "",
        thumbnail: "",
        title: "",
...
async function createImageMap(folder, imageMap) {
  ...
  const list = await createImageMapHelper(folder, [], 0).then((listResult) => {
    ...
    console.log(
      `createImageMapHelper done in ${elapsedTimeSeconds.toFixed(2)} seconds`
    );
    return listResult;
  });
  ...
  console.log(
    `createImageMap done in ${elapsedTimeSeconds.toFixed(2)} seconds`
  );
...
async function createImageMapHelper(folder, pathList, depth) {
  ...
  if (depth > 10) {
    //10 is an arbitrary number/cap to stop infinite loops beyond 10 layers of nesting
    console.error(
      "ERROR: Recursion limit reached, stopping further directory exploration."
    );
    return pathList;
  }
  ...
  try {
    ...
    const promises = result.prefixes.map((folderRef) => {
      //iterating over each subdirectory found in the folder
...
const postDataHard = () => {
  CHAPTERS.forEach((chapter) => {
    //iterating through chapters array
...
const postChapterData = async (chapter) => {
  ...
  const chapterReference = db
    .collection(GRADE_NAME)
    .doc(chapterData.navigation);
...
const postLessonData = async (lesson, chapterReference) => {
  ...
  const lessonLanguageReference = chapterReference
    .collection(lessonData.navigation)
    .doc(LANGUAGE_CODE);

  try {
    await lessonLanguageReference.set(lessonData); //setting lessonLanguage metadata
    console.log(
      `\t${lessonData.navigation}-${LANGUAGE_CODE} set successfully!`
    );
  } catch (error) {
  ...
  masteryAndMinigames.forEach((currentObject) => {
    ...
    !duplicates[currentObjectName]
      ? (duplicates[currentObjectName] = 1)
      : duplicates[currentObjectName]++;
...
const postMasteryAndMinigameData = async (
  currentObject,
  lessonLanguageReference
) => {
  const masteryAndMinigamesReference = lessonLanguageReference
    .collection("masteryAndMinigames")
    .doc(currentObject.navigation);
